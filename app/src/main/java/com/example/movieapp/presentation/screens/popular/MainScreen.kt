...
@Composable
fun PopularMovieScreen(navController: NavHostController,
               popularMovieState: MutableStateFlow<PagingData<Results>>
) {
    val moviePagingItems = popularMovieState.collectAsLazyPagingItems()
    Box{
        LazyVerticalGrid(
            ...
            items(moviePagingItems.itemCount) { index ->
                if(moviePagingItems[index]?.adult==false){
                    AsyncImage(
                        model = "${MOVIE_IMAGE_URL}${BackdropSize.w300}/${moviePagingItems[index]?.posterPath}",
                        contentDescription = "",
                        modifier = Modifier
                            .padding(2.dp),
