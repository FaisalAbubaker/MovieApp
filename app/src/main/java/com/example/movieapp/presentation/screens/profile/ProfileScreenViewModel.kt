...
import com.example.movieapp.domain.user.GetSessionIdUseCase
import com.example.movieapp.domain.user.GetUserAccountUseCase
import com.example.movieapp.domain.user.GetUserTokenUseCase
    ...
    fun getUserToken(){
        viewModelScope.launch {
            when (val response = getUserTokenUseCase.invoke()){
                ...
                is UIState.Empty -> userTokenState.value = UIState.Empty(response.title)
    ...
    fun getSessionId(requestToken: String){
        viewModelScope.launch {
            when (val response = getSessionIdUseCase.invoke(requestToken)){
                ...
                is UIState.Empty -> userSessionState.value = UIState.Empty(response.title)
    ...
    fun getUserAccount(sessionId: String){
        viewModelScope.launch {
            when (val response = getUserAccountUseCase.invoke(sessionId)){
                ...
                is UIState.Empty -> userAccountState.value = UIState.Empty(response.title)
